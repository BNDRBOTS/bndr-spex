...
const appChecks = [
  ...
  ['rail account does not expose full email', /rail-account-button/.test(appHtml + styles) && !/writing-mode:\s*vertical-rl[\s\S]*rail-account/.test(styles)],
  ['workspace canvas scrolls', /\.app-canvas[\s\S]*overflow-y:\s*auto/.test(styles) && /\.spex-body[\s\S]*height:\s*100dvh/.test(styles)],
