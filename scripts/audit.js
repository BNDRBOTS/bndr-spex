...
const serverChecks = [
  ...
  ['Generated specs require UI/backend binding matrix', /component_backend_bindings[\s\S]*implementation matrix[\s\S]*UI components[\s\S]*backend contracts/.test(server) && /Assign UI components to matching backend responsibilities[\s\S]*request payload[\s\S]*response shape[\s\S]*persistence target/.test(server)],
