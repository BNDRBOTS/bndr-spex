...
const appChecks = [
  ...
  ['topbar account exposes full email safely', /account-chip/.test(appHtml + styles) && /els\.accountLabel\.textContent = email \|\| 'Account'/.test(appJs) && /text-overflow:\s*ellipsis/.test(styles)],
...
const serverChecks = [
  ...
  ['Structured schema validation', /schemaKeys[\s\S]*structured_schema[\s\S]*component_backend_bindings[\s\S]*validation_flags[\s\S]*failure_modes[\s\S]*fallback_recovery_logic[\s\S]*acceptance_criteria[\s\S]*meta_tag[\s\S]*Structured schema failed validation/],
  ['Merged SPEX prompt', /convert the customer goal_description into a complete build-ready system specification for the product described by the customer/],
  ['One-description backend input', /required_user_input[\s\S]*goal_description/],
  ['Derived output keys', /deterministic_derivation_logic[\s\S]*component_backend_bindings[\s\S]*payment_access_logic[\s\S]*final_schema/],
  ['Subject drift gates', /internalProductMarkers[\s\S]*subjectAdherenceFailures[\s\S]*generationGateFailures/],
  ['BNDR SPEX internal drift blocked', /Do not describe BNDR \| SPEX[\s\S]*workbench[\s\S]*internal routes/],
  ['Generated specs require implementation fallbacks', /failure_modes[\s\S]*fallback_recovery_logic[\s\S]*observability_support_logic[\s\S]*acceptance_criteria/.test(server) && /graceful fallback and failsafe behavior[\s\S]*provider\/model timeout[\s\S]*save-after-generation failure/.test(server)],
  ['Generated specs require UI/backend binding matrix', /component_backend_bindings[\s\S]*implementation matrix[\s\S]*UI components[\s\S]*backend contracts/.test(server) && /Assign the generated product UI components[\s\S]*request payload[\s\S]*response shape[\s\S]*persistence target/.test(server)],
  ...
  ['Support notification route', /\/api\/support\/notify[\s\S]*notifyDeveloperHandler/],
