...
function appBaseUrl() {
  return envValue('APP_BASE_URL', ['APP_ORIGIN']).replace(/\/$/, '');
}

function stripeSinglePriceId() {
  return envValue('STRIPE_SINGLE_PRICE_ID', ['STRIPE_ONE_TIME_PRICE_ID']);
}

function stripeMonthlyPriceId() {
  return envValue('STRIPE_MONTHLY_PRICE_ID');
}
...
async function createCheckoutSession(user, profile, plan) {
  ...
  const params = {
    ...
    success_url: `${baseUrl}/app.html?checkout=success&session_id={CHECKOUT_SESSION_ID}`,
    ...
    'custom_text[terms_of_service_acceptance][message]': `I agree to BNDR | SPEX Terms of Service: ${baseUrl}/terms.html`
  };
